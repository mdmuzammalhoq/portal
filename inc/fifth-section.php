...
<?php 
	...
	$query = "SELECT * FROM  tbl_title WHERE categories='Position_6' ORDER BY id desc limit 1";
	$ads = $db->select($query);
	if ($ads) {
...
<?php } } ?>
										...
										<ul>
<?php 
	$query = "SELECT * FROM tbl_news WHERE category='দ্য বুকস্' order by id desc limit 1  ";
	$s22 = $db->select($query);
	if ($s22) {
...
<?php } } ?>
<?php 
	$query = "SELECT * FROM tbl_news WHERE category='দ্য বুকস্-লিস্ট' order by id desc limit 4  ";
	$s22 = $db->select($query);
	if ($s22) {
		while ($result = $s22->fetch_assoc()) {
?>
...
<?php 
	...
	$query = "SELECT * FROM tbl_advertisement WHERE position='Position  5' ORDER BY id desc limit 1";
	$ads = $db->select($query);
	if ($ads) {
		while ($result = $ads->fetch_assoc()) {
...
?>
						<div class=" logo_add add_img">
							<img src="adm-pol/<?php echo $result['image']; ?>" alt="Position  5" />
						</div>
<?php } } ?>
		</div>	<!--one section end-->