...
<?php 
	...
	$query = "SELECT * FROM  tbl_title WHERE categories='Position_17' ORDER BY id desc limit 1";
	$ads = $db->select($query);
	if ($ads) {
...
<?php } } ?>
										...
										<ul>
<?php 
	$query = "SELECT * FROM tbl_news WHERE category='Cat Thirteen Lead' order by id desc limit 1  ";
	$s22 = $db->select($query);
	if ($s22) {
...
<?php } } ?>	
<?php 
	$query = "SELECT * FROM tbl_news WHERE category='Cat Thirteen List' order by id desc limit 4  ";
	$s22 = $db->select($query);
	if ($s22) {
		while ($result = $s22->fetch_assoc()) {
?>
...
<?php 
	...
	$query = "SELECT * FROM tbl_advertisement WHERE position='Position  13' ORDER BY id desc limit 1";
	$ads = $db->select($query);
	if ($ads) {
		while ($result = $ads->fetch_assoc()) {
...
?>
						<div class=" logo_add add_img">
							<img src="adm-pol/<?php echo $result['image']; ?>" alt="Position  13" />
						</div>
<?php } } ?>
		</div>	<!--one section end-->